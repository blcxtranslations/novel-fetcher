...
from utilities.utility_common import find_links, get_page, strip_unicode
...
def get_chapters(url, lower=None, upper=None):
    ...
    if lower is not None:
        links = [link for link in links if int(link[0]) >= lower]
        # links = filter(lambda link: int(link[0]) >= lower, links)
    if upper is not None:
        links = [link for link in links if int(link[0]) <= upper]
